...
pub trait Knowledgeable<T: Joinable + Clone + Debug>: Command<T> + FactManager + RuleManager<T> + Cache {
    fn new() -> Self;
...
pub trait Command<Data: Joinable + Clone + Debug>: Cache {
    type Language: Debug;
    fn query_sqlite_db(&self, cmds: &str) -> Data;
    fn query_all_sqlite_db(&self) -> Data; // get a table of the datas included
    ...
    fn infer_commands_from(&self, cmds: &PredicatAST) -> Vec<Cmd>;
